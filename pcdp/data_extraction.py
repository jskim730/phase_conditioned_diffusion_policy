...
# Hip joint indices for all four Ant legs (used by multi-joint / PCA labeling).
# These are the same angular DOFs used as gait oscillators in the MuJoCo Ant model.
# Adjust if your obs layout differs.
ANT_HIP_JOINT_INDICES: tuple[int, ...] = (19, 21, 23, 25)
...
def extract_phase_multi_joint(
    obs_seq: np.ndarray,
    joint_indices: Sequence[int] = ANT_HIP_JOINT_INDICES,
    *,
    smooth_sigma: float = 2.0,
) -> np.ndarray:
    """Extract phase by circularly averaging Hilbert phases across multiple joints.

    Each joint contributes a unit-complex phasor ``exp(i*phi_j)``.  The
    circular mean angle gives a phase estimate that is robust to any single
    joint having low amplitude or noise.  Falls back to the default single-joint
    method if no valid joint signal is found.
    """
    phasors = []
    for idx in joint_indices:
        if idx >= obs_seq.shape[-1]:
            continue
        signal = obs_seq[:, idx]
        if float(np.std(signal)) < 1e-6:
            continue
        phi = extract_phase(signal, smooth_sigma=smooth_sigma).astype(np.float64)
        phasors.append(np.exp(1j * phi))

    if not phasors:
        return extract_phase(obs_seq[:, ANT_PHASE_JOINT_IDX], smooth_sigma=smooth_sigma)

    mean_phase = np.angle(np.mean(phasors, axis=0))
    return ((mean_phase + 2 * np.pi) % (2 * np.pi)).astype(np.float32)


def extract_phase_pca(
    obs_seq: np.ndarray,
    joint_indices: Sequence[int] = ANT_HIP_JOINT_INDICES,
    *,
    smooth_sigma: float = 2.0,
) -> np.ndarray:
    """Extract phase from the first PCA component of multiple joint signals.

    The first principal component captures the dominant oscillatory mode across
    joints, which often has better SNR than any single joint alone.  Falls back
    to the default single-joint method if fewer than two valid joints are found.
    """
    signals = []
    for idx in joint_indices:
        if idx >= obs_seq.shape[-1]:
            continue
        sig = obs_seq[:, idx].astype(np.float64)
        sig = sig - sig.mean()
        if np.std(sig) > 1e-6:
            signals.append(sig / np.std(sig))  # z-score normalise

    if len(signals) < 2:
        return extract_phase(obs_seq[:, ANT_PHASE_JOINT_IDX], smooth_sigma=smooth_sigma)

    X = np.stack(signals, axis=1)           # (T, N_joints)
    X -= X.mean(axis=0)
    _, s, Vt = np.linalg.svd(X, full_matrices=False)
    pc1 = X @ Vt[0]                         # (T,) first PC scores

    return extract_phase(pc1.astype(np.float32), smooth_sigma=smooth_sigma)


PhaseMethod = Literal["hilbert", "multi_joint", "pca"]


def extract_phase_with_method(
    obs_seq: np.ndarray,
    method: PhaseMethod = "hilbert",
    *,
    joint_indices: Sequence[int] = ANT_HIP_JOINT_INDICES,
    smooth_sigma: float = 2.0,
    phase_joint_idx: int = ANT_PHASE_JOINT_IDX,
) -> np.ndarray:
    """Dispatch to the requested phase labeling method.

    Args:
        obs_seq: ``(T, obs_dim)`` observation array.
        method: One of ``"hilbert"`` (default single-joint Hilbert),
            ``"multi_joint"`` (circular mean over hip joints), or
            ``"pca"`` (first PCA component of hip joints).
    """
    if method == "hilbert":
        return extract_phase(obs_seq[:, phase_joint_idx], smooth_sigma=smooth_sigma)
    if method == "multi_joint":
        return extract_phase_multi_joint(obs_seq, joint_indices, smooth_sigma=smooth_sigma)
    if method == "pca":
        return extract_phase_pca(obs_seq, joint_indices, smooth_sigma=smooth_sigma)
    raise ValueError(f"Unknown phase method {method!r}. Choose 'hilbert', 'multi_joint', or 'pca'.")
...
def relabel_phases(
    ...
    *,
    ...
    """Return a copy of demos with phases recomputed using a different labeling method.

    Does NOT require re-downloading or re-extracting demos — just recomputes the
    ``phases`` array from the already-saved ``observations``.  Use this to run
    phase-labeling ablations on top of an existing ``demos_ant.npz``.

    Args:
        demos: Dict loaded from ``demos_ant.npz`` (numpy arrays).
        method: ``"hilbert"``, ``"multi_joint"``, or ``"pca"``.

    Returns:
        Shallow copy of ``demos`` with ``phases`` replaced.

    Example::

        from pcdp.data_extraction import relabel_phases
        import numpy as np

        demos_raw = dict(np.load("demos_ant.npz"))
        demos_pca = relabel_phases(demos_raw, "pca")
        demos_mj  = relabel_phases(demos_raw, "multi_joint")
    """
    demos = dict(demos)  # shallow copy — only phases will be replaced
    n_demos = len(demos["observations"])
    new_phases = np.zeros_like(demos["phases"])

    for i in range(n_demos):
        length = int(demos["episode_lengths"][i])
        obs_seq = demos["observations"][i, :length]
        new_phases[i, :length] = extract_phase_with_method(
            obs_seq,
            method,
    ...
    demos["phases"] = new_phases
    print(f"relabel_phases: {n_demos} episodes re-labelled with method='{method}'")
    return demos
...
def extract_demos_from_episodes(
    ...
    *,
    ...
    Args:
        phase_method: Phase labeling strategy — ``"hilbert"`` (default),
            ``"multi_joint"``, or ``"pca"``.  See :func:`extract_phase_with_method`.
    """
    print(f"Phase 1: episodes 처리 (max {config.max_episodes_to_process}) [method={phase_method}]")
    ...
    for ep_i, ep in enumerate(episodes):
        ...
        phases = extract_phase_with_method(obs_seq, phase_method, smooth_sigma=config.smooth_sigma)
        # Quality metrics are still computed on the canonical single joint for consistency
