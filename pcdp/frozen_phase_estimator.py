...
def compute_snr_weights(phase: torch.Tensor, eps: float = 1e-6) -> torch.Tensor:
    """Per-step SNR weights from phase velocity consistency.

    Steps where instantaneous velocity is close to the episode mean get high weight;
    erratic or near-zero-velocity steps get down-weighted.

    Args:
        phase: (B, PH) raw phase angles in radians.

    Returns:
        weights: (B, PH) non-negative weights, mean-normalised to 1.0 per episode.
    """
    if phase.shape[1] < 2:
        return torch.ones_like(phase)

    # Wrapped instantaneous velocity (radians per step) in [-pi, pi]
    dphi = torch.atan2(
        torch.sin(phase[:, 1:] - phase[:, :-1]),
        torch.cos(phase[:, 1:] - phase[:, :-1]),
    )  # (B, PH-1)

    mean_vel = dphi.mean(dim=1, keepdim=True)          # (B, 1)
    std_vel = dphi.std(dim=1, keepdim=True).clamp(min=eps)  # (B, 1)

    # z-score deviation from mean velocity
    z = (dphi - mean_vel).abs() / std_vel              # (B, PH-1)
    w = torch.exp(-z)                                  # (B, PH-1)

    # Pad first step (no predecessor): weight = 1.0
    w0 = torch.ones(phase.shape[0], 1, device=phase.device, dtype=phase.dtype)
    w = torch.cat([w0, w], dim=1)                      # (B, PH)

    # Normalise so mean weight per episode = 1 (preserves overall loss scale)
    w = w / w.mean(dim=1, keepdim=True).clamp(min=eps)
    return w.detach()


def snr_weighted_circular_loss(
    pred_sincos: torch.Tensor,
    target_sincos: torch.Tensor,
    weights: torch.Tensor,
) -> torch.Tensor:
    """Weighted circular loss: ``mean(w * (1 - dot(target, pred)))``."""

    pred_sincos = F.normalize(pred_sincos, dim=-1, eps=1e-8)
    target_sincos = F.normalize(target_sincos, dim=-1, eps=1e-8)
    dot = (pred_sincos * target_sincos).sum(dim=-1).clamp(-1.0, 1.0)  # (B, PH)
    return (weights * (1.0 - dot)).mean()


def phase_velocity_loss(
    pred_sincos: torch.Tensor,
    target_sincos: torch.Tensor,
) -> torch.Tensor:
    """Phase velocity consistency loss.

    Penalises differences in the discrete phase derivative (wrapped, in radians).
    This encourages the estimator to match not just instantaneous phase but also
    the rate of phase change.
    """
    if pred_sincos.shape[1] < 2:
        return torch.zeros((), device=pred_sincos.device)

    pred_angle = torch.atan2(pred_sincos[..., 1], pred_sincos[..., 0])    # (B, PH)
    target_angle = torch.atan2(target_sincos[..., 1], target_sincos[..., 0])

    # Wrapped velocity in [-pi, pi]
    pred_vel = torch.atan2(
        torch.sin(pred_angle[:, 1:] - pred_angle[:, :-1]),
        torch.cos(pred_angle[:, 1:] - pred_angle[:, :-1]),
    )  # (B, PH-1)
    target_vel = torch.atan2(
...
def phase_sync_loss(
    ...
    *,
    ...
    Args:
        snr_weighted: If True, down-weight steps with erratic phase velocity.
        lambda_velocity: Weight on the phase velocity consistency term.  Set to
            0.0 to disable the velocity loss.
    """
    target = phase_target_sincos(phase)  # (B, PH, 2)
    pred = estimator(obs, x0_hat)        # (B, PH, 2)

    if snr_weighted:
        weights = compute_snr_weights(phase)  # (B, PH)
        position_loss = snr_weighted_circular_loss(pred, target, weights)
    else:
        position_loss = circular_sincos_loss(pred, target)

    if lambda_velocity > 0.0:
        vel_loss = phase_velocity_loss(pred, target)
        return position_loss + lambda_velocity * vel_loss
    ...
    for epoch in range(num_epochs):
        ...
        for batch in train_loader:
            ...
            if use_phase and lambda_phase > 0:
                x0_hat = predict_x0_from_epsilon(noisy_action, noise_pred, t, noise_scheduler)
                p_loss = phase_sync_loss(
                    estimator, obs, x0_hat, phase,
