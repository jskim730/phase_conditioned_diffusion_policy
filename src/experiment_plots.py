...
def plot_table1_reward_per_step_comparison(
    ...
    *,
    ...
    """Save a Table-1 companion figure comparing in-distribution reward/step.

    This plot directly visualizes the locomotion-quality metric from Table 1 for
    all three policies at the training-mean command frequency.
    """
    output_path = Path(output_path)
    ...
    values_by_model = {
        key: np.asarray(
            [
                _metric_value(result, "reward_per_step")
                for result in table1_results[key]
            ],
            dtype=np.float32,
        )
    ...
    spreads = []
    for key in model_keys:
        mean, spread = _mean_spread(values_by_model[key], interval=interval)
        means.append(mean)
    ...
    ax.bar(
        ...
        yerr=spreads,
        capsize=5,
        color=[colors[key] for key in model_keys],
        alpha=0.82,
        edgecolor="black",
        linewidth=0.8,
    )
    for idx, key in enumerate(model_keys):
        ...
        jitter = np.linspace(-0.08, 0.08, num=max(len(vals), 1))[: len(vals)]
        ax.scatter(
    ...
    ax.set_xticks(x)
    ax.set_xticklabels([labels[key] for key in model_keys])
    ax.set_ylabel("Reward / step")
    title = "In-distribution locomotion quality"
    if freq_hz is not None:
        title += f" @ {freq_hz:.3f} Hz"
    ax.set_title(title)
