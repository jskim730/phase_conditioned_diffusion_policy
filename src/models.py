...
Step 2 (Vanilla DP) — global_cond만 사용 (= flattened obs window).
Step 3+ (Phase-Conditioned) — 동일 architecture를 그대로 사용.
  Periodic phase encoding은 global_cond에 (cos φ, sin φ) 2차원 concat.
Step 4 (Phase Trajectory Conditioning) — 새 클래스 PhaseConditionedUnet1D 추가 예정.

기반: Chi et al. 2023, "Diffusion Policy: Visuomotor Policy Learning via Action Diffusion" (RSS).
"""
...
def build_periodic_phase_dp_model(
    ...
    """Step 3: Periodic Phase Encoding.

    Vanilla DP와 동일 architecture. 차이는 global_cond_dim에 +2 (cos φ, sin φ).
    Chunk 첫 step의 phase만 사용 (PAPL 스타일, step-wise condition).
    Per-step trajectory conditioning은 Step 4에서 별도 클래스로 추가.
    """
    model = ConditionalUnet1D(
        input_dim=act_dim,
