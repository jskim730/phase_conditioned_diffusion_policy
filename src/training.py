...
DDPM training (ε-prediction) + EMA + cosine LR + grad clip.
Vanilla DP, Periodic Phase, Phase Trajectory 모두 동일한 loop 사용 —
차이는 모델의 conditioning 방식뿐이라 train 함수가 batch에서 어떤 키를
condition으로 뽑는지만 callable로 바꿈.
"""
...
# Step 4 (Phase Trajectory Conditioning) 에서는 per_step_cond를 채우는
# trajectory_phase_cond_fn 추가 예정.
